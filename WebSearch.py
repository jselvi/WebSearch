...
def query_bing(key,query):
	...
	url = 'https://api.datamarket.azure.com/Data.ashx/Bing/Search/Web?Query=%27'+query+'%27&$top=1000&$format=json'
...
if len(options.bing_api) == 0:
	print "Set up a Bing API Key: https://datamarket.azure.com/dataset/bing/search"
        exit()
	...
	for num_IP in range(num_ipbegin, num_iplast+1):
		...
		try:
			...
			while len(response) > 0:
				oldlen = len(hosts)
				for res in response:
					hostname = res['Url'].split("/")[2].split(":")[0]
